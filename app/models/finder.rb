class Finder
  ...
  require 'open-uri'
  require 'kconv'
  attr_accessor :f_from,:f_to,:time_required,:d_time,:platform_number,:platform_name,:a_time,:price,:bording_number,:next_bus_params
  ...
  def get_next_bus
    href=@nokogiri.css("a.route_time_transition_prev")[0].attributes["href"].value
    f_date_year=href.match(/f_date_Year=\d*/)[0].split("=")[1]
    f_date_month=href.match(/f_date_Month=\d*/)[0].split("=")[1]
    f_date_day=href.match(/f_date_Day=\d*/)[0].split("=")[1]
    f_hour=href.match(/f_hour=\d*/)[0].split("=")[1]
    f_min=href.match(/f_min=\d*/)[0].split("=")[1]
    return [f_date_year,f_date_month,f_date_day,f_hour,f_min]
  end
end